...
<p align="center">
    <img src="images/knapsack.png" alt="Knapsack result table automatically generated by AutoExpe.jl" width="300"/>
</p>
...
This  dictionary  describes  one  result  column  of  the  table.  The
*entries* displayed in each row of a column can be obtained from:
* only one *value*  from the result files. In that  case, the entry is
...
* several *values*  from the result files. In that  case, all required
  values are read  from the result files and a  function is applied to
  them to compute the entry displayed  in the column (e.g., if we want
  to compute the  mean resolution time of two  different methods, `m1`
  and `m2`).

To identify  a value in  a result  file used to  obtain an entry  in a
column of a
latex table, we must specify:
* the name of the value in the result files (e.g., `time`);
* the resolution method which provided this value (e.g., `m1` and `m2`).
...
Boolean true  if a vertical line  is drawn after this  column (default
value: `false`).

#### function (optional)

Function  used to  compute the  value in  the column.  Currently, only
`"gap"` and  `"mean"` are available but  it should be possible  in the
future  for the  user  to define  and specify  them  similarly to  the
resolution methods.
...
Used when the column  is within a group of columns.  It is possible to
set the best value  of a group of columns in bold  (e.g., if the group
represents the resolution time of several methods). This entry is true
