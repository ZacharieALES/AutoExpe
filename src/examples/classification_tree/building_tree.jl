...
Entrées :
- instancePath : path of the text file which includes the data set
- mergePercentage : remaining % of data after the merge
- D : Nombre maximal de séparations d'une branche (profondeur de l'arbre - 1)
- lambd : poids de l'objectif d'interprétabilité (plus lambd est grand, plus l'arbre sera simple/interprétable)
...
"""
function build_tree(input::Dict{String, Any})

    dataSetPath = input["instancePath"]
    percentage = input["mergePercentage"]
    D = input["D"]
    lambd = input["lambd"]

    multivariate = false
    if haskey(input, "multivariate")
        multivariate = input["multivariate"]
    end 

    time_limit = -1
    if haskey(input, "time_limit")
    ...
    clusters = simpleMerge(X_train, Y_train, percentage) 
...
#        @objective(m, Max, (1-lambd) * sum(length(clusters[i].dataIds) * u_at[i, 1] for i in 1:clusterCount) - lambd * sum(s[j, t] for j in 1:featuresCount for t in 1:sepCount))
    else
        @objective(m, Max, sum(length(clusters[i].dataIds) * u_at[i, 1] for i in 1:clusterCount))
#        @objective(m, Max, (1-lambd) * sum(length(clusters[i].dataIds) * u_at[i, 1] for i in 1:clusterCount) - lambd * sum(a[j, t] for j in 1:featuresCount for t in 1:sepCount))
    end
    ...
    if primal_status(m) == MOI.FEASIBLE_POINT
        ...
        results["objective"] = objective_value(m)
        results["bestBound"] = JuMP.objective_bound(m)
        ...
        results["T.D"] = T.D
        results["gap"] = gap
        results["train_error"] = 100 * prediction_errors(T,X_train,Y_train)/size(X_train, 1)
        results["test_error"] = 100 * prediction_errors(T,X_test,Y_test)/size(X_test, 1)
    end 

    results["resolutionTime"] = resolution_time
    results["dataSet"] = split(split(input["instancePath"], "data/")[2], ".txt")[1]
    results["mergePercentage"] = percentage
    results["D"] = D
    results["multivariate"] = multivariate
    results["clustersCount"] = clusterCount
